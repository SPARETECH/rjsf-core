...
function retrieveSchema(schema) {
  ...
  if ("if" in resolvedSchema) {
    ...
    var conditionalSchema = (0, _validate.isValid)(expression, formData) ? then : otherwise;
  ...
  if (allOf) {
    for (var i = 0; i < allOf.length; i++) {
      ...
      if ("if" in allOfSchema) {
        allOfSchema = (0, _validate.isValid)(allOfSchema["if"], formData) ? allOfSchema.then : allOfSchema["else"];
      }
